...
import { IconoGema, IconoBendicion, IconoCaja, IconoRombo } from "./icons";

export default function Taller() {
  return (
    <section id="taller" className="relative overflow-hidden bg-vino-950 py-24 text-marfil-50 lg:py-32">
      <div
        ...
        <div className="lg:sticky lg:top-32 lg:self-start">
          <Reveal>
            ...
            <h2 className="mt-6 font-display text-4xl font-bold leading-tight sm:text-5xl">
              Hecho a mano,
              <br />
              <span className="font-quote font-medium italic text-oro-300">cuenta por cuenta</span>
            </h2>

            <p className="mt-6 max-w-lg leading-relaxed text-marfil-100/70">
              Cada pieza nace en nuestro taller artesanal, donde combinamos técnicas tradicionales
              con materiales nobles. Desde la selección de cuentas hasta la bendición final,
              cada paso se realiza con devoción y precisión.
            </p>
          ...
          {PROCESO.map((p, i) => (
            <Reveal key={p.num} delay={i * 110}>
              <div className="group flex gap-6 border-b border-oro-400/15 py-9 first:pt-0">
                ...
                <span>
                  <h3 className="font-display text-xl font-semibold text-marfil-50">{p.titulo}</h3>
                  <p className="mt-2.5 leading-relaxed text-marfil-100/60">{p.texto}</p>
                </span>
              </div>
            </Reveal>
          ))}
